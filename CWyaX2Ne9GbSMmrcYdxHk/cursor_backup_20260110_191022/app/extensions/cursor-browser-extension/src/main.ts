...
class PlaywrightMcpProvider implements McpProvider {
	...
	async initialize(): Promise<void> {
		try {
			...
			// Get workspace-scoped storage directory if available
			let storageDir: string | undefined;
			if (this.extensionContext.storageUri) {
				// Add 'browser-session' subdirectory to allow for other workspace data
				storageDir = path.join(
					this.extensionContext.storageUri.fsPath,
					'browser-session'
				);
				PlaywrightLogger.info(
					`Using workspace storage directory: ${storageDir}`
				);
			} else {
				PlaywrightLogger.info(
					'No workspace storage available, will use temporary storage'
				);
			}

			// Initialize browser manager with config and storage directory
			this.browserManager = new BrowserManager(this.currentConfig, storageDir);
