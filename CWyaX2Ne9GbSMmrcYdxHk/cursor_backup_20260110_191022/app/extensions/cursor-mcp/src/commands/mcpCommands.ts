...
import { CallToolResultSchema, Progress, ProgressNotification, ProgressNotificationSchema, ToolListChangedNotification, ToolListChangedNotificationSchema, ListRootsRequest, ListRootsRequestSchema, ElicitRequest, ElicitRequestSchema, Tool, ResourceListChangedNotification, ResourceListChangedNotificationSchema, ResourceUpdatedNotification, ResourceUpdatedNotificationSchema } from '@modelcontextprotocol/sdk/types.js';
...
import { MCPOAuthClientProvider, OAuthClientInformationWithMetadata, fetchScopesFromWellKnown } from './mcp/oauth.js';
...
class ClientToMcpClientAdapter implements McpClient {
	...
	private readonly resourcesCache: AsyncCache<Array<{ uri: string; name?: string; description?: string; mimeType?: string; annotations?: Record<string, unknown> }>>;
	...
	constructor(
		...
		// Cache resources for 24 hours (same as tools)
		this.resourcesCache = new AsyncCache<Array<{ uri: string; name?: string; description?: string; mimeType?: string; annotations?: Record<string, unknown> }>>(1000 * 60 * 60 * 24);
	}
	...
	invalidateResourcesCache(): void {
		this.resourcesCache.invalidate();
	}
	...
	async listResources(_ctx: Context): Promise<{
		...
		return await this.resourcesCache.get(async () => {
			return await this.withSessionRetry(async () => {
	 ...
	 */
	async getToolsForUI(): Promise<{ name: string; description: string; parameters: string; outputSchema?: string }[]> {
		return await this.withSessionRetry(async () => {
			const capabilities = this.client.getServerCapabilities() ?? { tools: false };
			if (!capabilities.tools) {
				return [];
			...
			return toolsResult.tools.map((tool: Tool) => {
				...
				// Serialize outputSchema if present
				const outputSchema = tool.outputSchema ? JSON.stringify(tool.outputSchema) : undefined;
	...
	async listOfferingsForUI(): Promise<{
		tools: { name: string; description: string; parameters: string; outputSchema?: string }[];
	...
	/**
	 * Wrapper for client operations that handles session termination errors.
	 * If a session termination is detected (HTTP 404 or JSON-RPC -32000 with session message),
	 * re-initializes the session and retries.
	 */
	private async withSessionRetry<T>(
		...
		try {
			...
			// Check if this is a session termination error (HTTP 404 or JSON-RPC session error)
			if (
				isSessionTerminatedError(error, serverInstance.serverInfo) &&
				retryCount < 1
			) {
				getLogger(this.identifier).info(
					"Detected session termination, re-initializing connection"
				);
...
/** Map from identifier to set of subscribed resource URIs (for cleanup) */
...
/**
 * Map from server identifier to pending client creation.
 * Stores the serverInfo and a promise that resolves when the client is ready.
 * This allows listOfferings to wait for a client that's being created.
 */
const pendingClientCreations = new Map<string, {
	serverInfo: MCPServerInfo;
...
let cachedBetterMcpStartupErrorHandling: boolean | undefined;

// Local McpManager instance that tracks all clients
const mcpManager = new McpManager({});

/**
 * Check if the better MCP startup error handling feature flag is enabled.
 * When enabled, listOfferings will wait for pending client creations and
 * return empty results gracefully instead of throwing errors.
 *
 * The result is cached after the first call to avoid round-trip time (RTT)
 * to the browser process on subsequent calls.
 */
async function isBetterMcpStartupErrorHandlingEnabled(): Promise<boolean> {
	if (cachedBetterMcpStartupErrorHandling !== undefined) {
		return cachedBetterMcpStartupErrorHandling;
	}
	cachedBetterMcpStartupErrorHandling = await vscode.cursor.checkFeatureGate('better_mcp_startup_error_handling');
	return cachedBetterMcpStartupErrorHandling;
}
...
/**
 * Check if an error indicates a terminated/invalid MCP session.
 ...
 *
 * However, some servers return a JSON-RPC error with code -32000 and a session-related
 * message instead of HTTP 404. We handle both cases.
 *
 * We check:
 * 1. Is this a streamableHttp server? (Only they use session IDs)
 * 2. Is this an HTTP 404 error, OR a JSON-RPC error indicating invalid session?
 *
...
function isSessionTerminatedError(error: unknown, serverInfo: MCPServerInfo): boolean {
	...
	// Some servers return a JSON-RPC error instead of HTTP 404 for invalid sessions.
	// The MCP SDK formats these as "MCP error -32000: <message>".
	// We check for this pattern with a session-related message.
	if (isJsonRpcSessionError(error.message)) {
		return true;
	}
...
/**
 * Check if an error message indicates an invalid session from a JSON-RPC -32000 error.
 * The MCP SDK formats these errors as "MCP error -32000: <message>".
 * We also check for raw JSON-RPC responses as a fallback.
 */
function isJsonRpcSessionError(errorMessage: string): boolean {
	// The MCP SDK formats JSON-RPC errors as "MCP error <code>: <message>"
	// Check for this pattern with code -32000 and session-related message
	const mcpErrorMatch = errorMessage.match(/MCP error -32000:\s*(.+)/i);
	if (mcpErrorMatch) {
		const mcpMessage = mcpErrorMatch[1];
		if (/session/i.test(mcpMessage)) {
			return true;
		}
	}

	// Also check for raw JSON-RPC error in case some code paths include it
	const jsonMatch = errorMessage.match(/\{[\s\S]*"jsonrpc"[\s\S]*\}/);
	if (jsonMatch) {
		try {
			const jsonRpc = JSON.parse(jsonMatch[0]);
			if (
				jsonRpc?.error?.code === -32000 &&
				typeof jsonRpc?.error?.message === 'string' &&
				/session/i.test(jsonRpc.error.message)
			) {
				return true;
			}
		} catch {
...
/**
 * Re-initialize a streamableHttp connection after a session termination.
 * This handles both HTTP 404 responses and JSON-RPC -32000 session errors.
...
async function reinitializeStreamableHttpConnection(
	...
	getLogger(identifier).info("Re-initializing connection after session termination");
	...
	if (existingAdapter) {
		...
		if (newServerInstance) {
			...
			// Invalidate caches since the new session may have different tools/instructions/resources
			existingAdapter.invalidateToolsCache();
			existingAdapter.invalidateInstructionsCache();
			existingAdapter.invalidateResourcesCache();
...
function cleanupServer(identifier: string): void {
	...
	if (!existingProcess) {
		// Also clean up any pending creations even if server not in servers Map
		// (only relevant when feature flag is enabled, but safe to do always)
		const pending = pendingClientCreations.get(identifier);
		if (pending) {
			pending.reject(new Error('Server cleanup requested during creation'));
			pendingClientCreations.delete(identifier);
		}
		return;
	}
	...
	try {
		// Unsubscribe from all resources before closing (fire-and-forget, best effort)
		const uris = subscribedResources.get(identifier);
		if (uris && uris.size > 0) {
			for (const uri of uris) {
				existingProcess.client.unsubscribeResource({ uri })
					.then(() => getLogger(identifier).info(`Unsubscribed from resource: ${uri}`))
					.catch((unsubError) => {
						// Ignore unsubscribe errors during cleanup
						getLogger(identifier).debug(`Error unsubscribing from resource ${uri}:`, unsubError);
					});
			}
			subscribedResources.delete(identifier);
		}
	...
	// Clean up pending creations (only relevant when feature flag is enabled, but safe to do always)
	pendingClientCreations.delete(identifier);
...
/**
 * Subscribe to resources if the server supports it.
 * This enables receiving notifications/resources/updated notifications when resources change.
 */
async function subscribeToResourcesIfSupported(identifier: string, client: Client): Promise<void> {
	try {
		const capabilities = client.getServerCapabilities() ?? { resources: false };
		// Check if server supports resources at all
		// Note: We check if resources is an object (not false) rather than checking for subscribe property
		// because some MCP SDK versions may not properly pass through the subscribe property to the client.
		// If the server doesn't support subscriptions, the subscribeResource call will fail and we'll catch it.
		if (!capabilities.resources || typeof capabilities.resources !== 'object') {
			getLogger(identifier).debug(`Server does not support resources, skipping subscription`);
			return;
		}

		// List resources and subscribe to each one
		const resourcesResult = await client.listResources();
		if (!resourcesResult?.resources?.length) {
			getLogger(identifier).debug(`No resources to subscribe to`);
			return;
		}
		...
		for (const resource of resourcesResult.resources) {
			try {
				...
				getLogger(identifier).info(`Subscribed to resource: ${resource.uri}`);
			} catch (subError) {
				// Log but continue - some resources may not support subscription
				getLogger(identifier).debug(`Failed to subscribe to resource ${resource.uri}:`, subError);
			}
		}

		if (uris.size > 0) {
			subscribedResources.set(identifier, uris);
			getLogger(identifier).info(`Subscribed to ${uris.size} resources`);
		}
	} catch (error) {
		// Non-fatal - log and continue
		getLogger(identifier).debug(`Error subscribing to resources:`, error);
	}
}
...
async function getOrCreateClient(identifier: string, serverInfo: MCPServerInfo): Promise<Client> {
	...
	const client = new Client(
		...
		{
			capabilities: {
				// Support form-based elicitation (user input forms)
				elicitation: { form: {} },
				roots: {
					listChanged: false
				}
	...
	client.setNotificationHandler(ResourceListChangedNotificationSchema, (_notification: ResourceListChangedNotification) => {
		getLogger(identifier).info(`ResourceListChangedNotification received`);
		// Invalidate the resources cache for this adapter
		const adapter = mcpManager.getClient(identifier);
		if (adapter instanceof ClientToMcpClientAdapter) {
			adapter.invalidateResourcesCache();
		}
		vscode.commands.executeCommand('mcp.resourceListChanged', { identifier });
	});

	client.setNotificationHandler(ResourceUpdatedNotificationSchema, (notification: ResourceUpdatedNotification) => {
		getLogger(identifier).info(`ResourceUpdatedNotification received for URI: ${notification.params?.uri}`);
		// Invalidate the resources cache for this adapter since a specific resource changed
		const adapter = mcpManager.getClient(identifier);
		if (adapter instanceof ClientToMcpClientAdapter) {
			adapter.invalidateResourcesCache();
		}
	...
	client.setRequestHandler(ElicitRequestSchema, async (request: ElicitRequest) => {
		...
		// ElicitRequest params can be form mode (has requestedSchema) or url mode (has url/elicitationId)
		const params = request.params;
		// Form mode has requestedSchema property; url mode has url/elicitationId instead
		const isFormMode = 'requestedSchema' in params;

		const elicitationProvider = currentElicitationProvider.get(identifier);
		// ScopedMcpElicitationProvider only supports form mode (requires requestedSchema)
		if (elicitationProvider && isFormMode) {
			try {
				const response = await elicitationProvider.elicit({
					message: params.message,
					requestedSchema: params.requestedSchema as {
						type: "object";
		...
		return new Promise((resolve) => {
			...
			// Build elicitationRequest based on mode: form mode has requestedSchema, url mode has url/elicitationId
			const elicitationRequest = {
				id: requestId,
				message: params.message,
				...(isFormMode
					? { requestedSchema: params.requestedSchema }
					: { url: (params as { url: string; elicitationId: string }).url, elicitationId: (params as { url: string; elicitationId: string }).elicitationId }),
	...
	// Subscribe to resources if the server supports it
	subscribeToResourcesIfSupported(identifier, client);
...
async function createAndConnectTransportFor(
	...
	if (serverInfo.type === 'streamableHttp') {
		...
		// Resolve OAuth scopes (config takes priority over well-known)
		let scopes: string[] | undefined;
		const scopesEnabled = await vscode.cursor.checkFeatureGate('mcp_oauth_scopes_enabled');
		if (scopesEnabled) {
			if ('auth' in serverInfo && serverInfo.auth) {
				scopes = serverInfo.auth.scopes;
				if (!scopes) {
					getLogger(identifier).info(`No scopes in config, fetching from well-known endpoint`);
				}
			}
			if (!scopes) {
				scopes = await fetchScopesFromWellKnown(serverInfo.serverUrl);
				if (scopes) {
					getLogger(identifier).info(`Using OAuth scopes: ${scopes.join(', ')}`);
				}
			}
		}
		...
		const authProvider = new MCPOAuthClientProvider({
			...
			identifier,
			needsAuthCallback: (authorizationUrl: URL) => {
			...
			staticClientInformation: staticClientInfo,
			scopes,
		});
		...
		try {
			...
			getLogger(identifier).warn(`Error connecting to streamableHttp server, falling back to SSE:`, error);
...
async function listOfferings(identifier: string) {
	let serverInstance = servers.get(identifier);

	// Feature-flagged: graceful handling for pending client creations
	const useBetterErrorHandling = await isBetterMcpStartupErrorHandlingEnabled();

	// Re-check in case server was created during the feature flag await
	if (!serverInstance) {
		serverInstance = servers.get(identifier);
	}

	// If server isn't in the servers Map yet, check if there's a pending creation
	if (!serverInstance && useBetterErrorHandling) {
		const pending = pendingClientCreations.get(identifier);
		if (pending) {
			getLogger(identifier).info(`Server creation in progress, waiting for completion`);
			try {
				await pending.promise;
				// After waiting, the server should now be in the Map
				serverInstance = servers.get(identifier);
			} catch (error) {
				getLogger(identifier).warn(`Pending server creation failed:`, error);
				// Return empty results instead of throwing - the creation error
				// will be surfaced through the CreateClient action
				return { tools: [], prompts: [], resources: [] };
			}
		}
	}

	// If still not found, either return empty gracefully (new behavior) or throw (old behavior)
	if (!serverInstance) {
		if (useBetterErrorHandling) {
			// This can happen if listOfferings is called before CreateClient is even started
			getLogger(identifier).info(`Server not yet created, returning empty offerings`);
			return { tools: [], prompts: [], resources: [] };
		} else {
...
async function callTool(identifier: string, name: string, args?: Record<string, unknown>, toolCallId?: string, elicitationProvider?: ScopedMcpElicitationProvider) {
	...
	// Feature-flagged: wait for pending creations before throwing
	if (!serverInstance && await isBetterMcpStartupErrorHandlingEnabled()) {
		// Re-check in case server was created during the feature flag await
		serverInstance = servers.get(identifier);
		if (!serverInstance) {
			const pending = pendingClientCreations.get(identifier);
			if (pending) {
				getLogger(identifier).info(`Server creation in progress for callTool, waiting`);
				await pending.promise;
				serverInstance = servers.get(identifier);
			}
		}
...
async function getInstructions(identifier: string) {
	...
	if (!serverInstance && await isBetterMcpStartupErrorHandlingEnabled()) {
		...
		if (!serverInstance) {
			const pending = pendingClientCreations.get(identifier);
			if (pending) {
				getLogger(identifier).info(`Server creation in progress for getInstructions, waiting`);
				await pending.promise;
				serverInstance = servers.get(identifier);
			}
		}
...
async function getPrompt(identifier: string, name: string, args?: Record<string, unknown>) {
	...
	if (!serverInstance && await isBetterMcpStartupErrorHandlingEnabled()) {
		...
		if (!serverInstance) {
			const pending = pendingClientCreations.get(identifier);
			if (pending) {
				getLogger(identifier).info(`Server creation in progress for getPrompt, waiting`);
				await pending.promise;
				serverInstance = servers.get(identifier);
			}
		}
...
async function readResource(identifier: string, uri: string) {
	...
	if (!serverInstance && await isBetterMcpStartupErrorHandlingEnabled()) {
		...
		if (!serverInstance) {
			const pending = pendingClientCreations.get(identifier);
			if (pending) {
				getLogger(identifier).info(`Server creation in progress for readResource, waiting`);
				await pending.promise;
				serverInstance = servers.get(identifier);
			}
		}
...
registerAction(MCPActions.CallTool, async (args: { identifier: string, name: string, args: Record<string, unknown>, useLeaseElicitation: boolean, toolCallId?: string }, extra: ExtraContext) => {
	getLogger(args.identifier).info(`Handling CallTool action for tool '${args.name}' with toolCallId: ${args.toolCallId}`);

	// If toolCallId is provided, create a provider that calls back to the workbench
	// via the mcp.leaseElicitationRequest command. This uses the normal elicitation path
	// (currentElicitationProvider in the MCP SDK handler) rather than the UI fallback.
	// Note: toolCallId is used as the key for lease elicitation handler lookup in MCPService.
	// Note: We ONLY do this for NAL. The OAL uses this path so we have useLease to differentiate.
	let elicitationProvider: ScopedMcpElicitationProvider | undefined;
	if (args.toolCallId && args.useLeaseElicitation) {
		elicitationProvider = {
			async elicit(request: { message: string; requestedSchema: { type: 'object'; properties: Record<string, unknown>; required?: string[] } }): Promise<{ action: 'accept' | 'decline' | 'cancel'; content?: Record<string, unknown> }> {
				getLogger(args.identifier).info(`Calling mcp.leaseElicitationRequest for toolCallId: ${args.toolCallId}`);
				try {
					const response = await vscode.commands.executeCommand<{ action: 'accept' | 'decline' | 'cancel'; content?: Record<string, unknown> }>(
						'mcp.leaseElicitationRequest',
						{
					...
					getLogger(args.identifier).info(`mcp.leaseElicitationRequest returned: ${response?.action}`);
					return response ?? { action: 'decline' };
				} catch (error) {
					getLogger(args.identifier).error(`Error in mcp.leaseElicitationRequest:`, error);
	...
	return callTool(args.identifier, args.name, args.args, args.toolCallId, elicitationProvider);
});
...
registerAction(MCPActions.CreateClient, async (args: { identifier: string, serverInfo: MCPServerInfo }, extra: ExtraContext) => {
	...
	if (useBetterErrorHandling) {
		...
		const existingPending = pendingClientCreations.get(args.identifier);
		if (existingPending) {
			getLogger(args.identifier).info(`CreateClient already in progress, waiting for existing creation`);
			await existingPending.promise;
			getLogger(args.identifier).info(`CreateClient completed (from existing), server stored: ${servers.has(args.identifier)}`);
			return { success: true };
		}

		// Create a pending entry so listOfferings can wait for this creation
		let resolveCreation: (client: Client) => void;
		let rejectCreation: (error: Error) => void;
		const creationPromise = new Promise<Client>((resolve, reject) => {
			resolveCreation = resolve;
			rejectCreation = reject;
		});

		pendingClientCreations.set(args.identifier, {
		...
		try {
			...
			rejectCreation!(error instanceof Error ? error : new Error(String(error)));
			throw error;
		} finally {
			// Clean up the pending entry
			pendingClientCreations.delete(args.identifier);
		}
	} else {
		// Old behavior: just create the client directly
		const client = await getOrCreateClient(args.identifier, args.serverInfo);
		getLogger(args.identifier).info(`CreateClient completed, server stored: ${servers.has(args.identifier)}`);
		return { success: true };
	}
...
registerAction(MCPActions.ListToolsRaw, async (args: { identifier: string }, extra: ExtraContext) => {
	getLogger(args.identifier).info(`Handling ListToolsRaw action`);
	const adapter = mcpManager.getClient(args.identifier) as ClientToMcpClientAdapter | undefined;
	if (!adapter) {
		return { tools: [] };
	}
	const tools = await adapter.getTools({ withName: () => ({ withName: () => ({}) }) } as any);
	return { tools };
});
...
registerAction(MCPActions.ReloadClient, async (args: { identifier: string, serverInfo: MCPServerInfo }, extra: ExtraContext) => {
	...
	if (useBetterErrorHandling) {
		...
		// Create new client with provided server info (reuse CreateClient logic for pending tracking)
		let resolveCreation: (client: Client) => void;
		let rejectCreation: (error: Error) => void;
		const creationPromise = new Promise<Client>((resolve, reject) => {
			resolveCreation = resolve;
...
registerAction(MCPActions.LogoutServer, async (args: { identifier: string }, extra: ExtraContext) => {
	...
	try {
		...
		const oauthProvider = new MCPOAuthClientProvider({
			...
			needsAuthCallback: (_authorizationUrl: URL) => {
