...
/**
 * Fetches OAuth scopes_supported from the server's well-known endpoint.
 * Returns undefined if the endpoint is not available or doesn't contain valid scopes.
 */
export async function fetchScopesFromWellKnown(serverUrl: string): Promise<string[] | undefined> {
	try {
		...
		// Validate that scopes_supported is actually an array of strings
...
/**
 * Options for constructing an MCPOAuthClientProvider.
 * Using an options object prevents accidental argument swapping between string parameters.
 */
export interface MCPOAuthClientProviderOptions {
	context: vscode.ExtensionContext;
	serverUrl: string;
	identifier: string;
	needsAuthCallback: MCPOAuthNeedsAuthCallback;
...
export class MCPOAuthClientProvider implements OAuthClientProvider {
	...
	constructor(options: MCPOAuthClientProviderOptions) {
		...
		this.needsAuthCallback = options.needsAuthCallback;
	...
	async redirectToAuthorization(authorizationUrl: URL): Promise<void> {
		// Append scopes if provided and not already present
		if (this.scopes?.length && !authorizationUrl.searchParams.has('scope')) {
