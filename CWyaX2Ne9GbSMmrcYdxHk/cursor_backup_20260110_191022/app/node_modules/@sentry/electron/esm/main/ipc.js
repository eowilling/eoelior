import { debug, parseEnvelope, _INTERNAL_captureSerializedLog } from '@sentry/core';
import { getCurrentScope, getClient, captureEvent } from '@sentry/node';
...
function handleLogFromRenderer(client, options, log, contents) {
    const process = contents ? options?.getRendererName?.(contents) || 'renderer' : 'renderer';
...
function configureClassic(client, ipcUtil, options) {
    ...
    ipcMain.on(ipcUtil.createKey('structured-log'), ({ sender }, log) => handleLogFromRenderer(client, options, log, sender));
    const rendererStatusChanged = createRendererEventLoopBlockStatusHandler(client);
...
export { configureIPC };//# sourceMappingURL=http://go/sourcemap/sourcemaps/4ca9b38c6c97d4243bf0c61e51426667cb964bd0/node_modules/@sentry/electron/esm/main/ipc.js.map
