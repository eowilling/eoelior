...
function createOfflineStore(userOptions) {
    function log(...args) {
        core.debug.log('[Offline Store]:', ...args);
    }
    const options = {
        maxAgeDays: userOptions.maxAgeDays || 30,
        maxQueueSize: userOptions.maxQueueSize || 30,
        queuePath: userOptions.queuePath || path.join(electronNormalize.getSentryCachePath(), 'queue'),
    };
    const queue = new store.Store(options.queuePath, 'queue-v2', []);
    function removeBody(id) {
        fs.promises.unlink(path.join(options.queuePath, id)).catch(() => {
            // ignore
        });
    }
    ...
    async function insert(env, which, previousDate) {
        ...
        const id = core.uuid4();
        try {
            const data = core.serializeEnvelope(env);
            await fs.promises.mkdir(options.queuePath, { recursive: true });
            await fs.promises.writeFile(path.join(options.queuePath, id), data);
        }
        catch (e) {
    ...
    return {
        ...
        shift: async () => {
            ...
            if (request) {
                try {
                    const data = await fs.promises.readFile(path.join(options.queuePath, request.id));
                    removeBody(request.id);
                    lastShiftedDate = request.date;
                    return core.parseEnvelope(data);
                }
                catch (e) {
...
exports.createOfflineStore = createOfflineStore;//# sourceMappingURL=http://go/sourcemap/sourcemaps/4ca9b38c6c97d4243bf0c61e51426667cb964bd0/node_modules/@sentry/electron/main/transports/offline-store.js.map
