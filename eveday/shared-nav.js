...
(function () {
  ...
  function render() {
    const developerMode = window.localStorage && window.localStorage.getItem('developerMode') === 'true';
    if (!developerMode) {
      // 需求：最上面「工具集」只允許出現在開發者模式
      mount.innerHTML = '';
      return;
    }
    ...
    function isActive(href) {
      ...
      // 回入口在 eveday 裡通常不會是 active（保留一般比對即可）
  ...
  // 讓其他頁面可以在切換開發者模式時刷新導覽列
  window.__evedayRefreshNav = render;
  render();
})();
